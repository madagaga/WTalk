...
namespace coreJson
{
    public class JSON
    {
        ...
        internal static List<object> ParseAsList(string data)
        {
            return (List<object>)Parse(data);
        }

        public static T Deserialize<T>(string input) where T : new()
        {
            object o = new JsonParser().Parse(input);
            if (o is IDictionary)
            {
        ...
        private static object ParseObject(Type type, Dictionary<string, object> input)
        {
            ...
            foreach (var property in properties.Values)
            {
                ...
                if (input[property.Name] is IDictionary)
                    property.Set(result, ParseObject(property.UnderlyingType, (Dictionary<string, object>)input[property.Name]));
                else if (input[property.Name] is IList)
                    property.Set(result, ParseArray(property.UnderlyingType, (List<object>)input[property.Name]));
                else if(property.IsEnum)
                {
                    if (input[property.Name] != null)
                        property.Set(result, Enum.Parse(property.UnderlyingType, input[property.Name].ToString()));
                }                
                else
                    property.Set(result, Convert.ChangeType(input[property.Name], property.UnderlyingType));
            }
            return result;
        }
